function eqm = Model_LoM_PDF(p, np, func, eqm)
    ...
    % LoM_RHS(1,:) = (1 - p.gamma) * eqm.nu_hat(1) * arrayfun(eqm.lambda_omega_tilde_fY_interp_l, np.q) ...
    %             + (1 - p.gamma) * eqm.nu_hat(1) * ( 1 - eqm.lambda(1,:) ) .* eqm.omega_tilde(1,:) ...
    %             + (1 - p.gamma) * eqm.mu_hat(1,1) * arrayfun(eqm.omega_tilde_fX_interp_l, np.q) ...
    %             + (1 - p.gamma) * eqm.mu_hat(1,2) * arrayfun(eqm.omega_tilde_fX_interp_h, np.q) ...
    %             + p.gamma * p.m(1) * ( eqm.omega_tilde(1,:) + eqm.omega_tilde(2,:) );

    % LoM_RHS(2,:) = (1 - p.gamma) * eqm.nu_hat(2) * arrayfun(eqm.lambda_omega_tilde_fY_interp_h, np.q) ...
    %             + (1 - p.gamma) * eqm.nu_hat(2) * ( 1 - eqm.lambda(2,:) ) .* eqm.omega_tilde(2,:) ...
    %             + (1 - p.gamma) * eqm.mu_hat(2,2) * arrayfun(eqm.omega_tilde_fX_interp_h, np.q) ...
    %             + (1 - p.gamma) * eqm.mu_hat(2,1) * arrayfun(eqm.omega_tilde_fX_interp_l, np.q) ...
    %             + p.gamma * p.m(2) * ( eqm.omega_tilde(1,:) + eqm.omega_tilde(2,:) );
